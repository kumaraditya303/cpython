...
/* Look up the special method name on the type of cls and call it with one
   argument, without creating a bound method.  In the free-threaded build
   this avoids touching the reference counts of the (shared) method and
   class on every call: the possibly deferred reference to the method is
   registered on the thread state so that the GC keeps it alive.
   Returns 1 and sets *res if the method was found, 0 if it is missing and
   -1 on error. */
static int
