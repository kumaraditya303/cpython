...
/*[clinic end generated code: output=0e107cbb7f72aa7b input=43a1b423c2d95bfa]*/
{
    ...
    // Stop the world and merge all the tasks from other threads
    // into the current thread's tasks list.
    _PyEval_StopTheWorld(interp);
    // TODO check if HEAD_LOCK/HEAD_UNLOCK is needed
    _PyThreadStateImpl *current = (_PyThreadStateImpl *)_PyThreadState_GET();
    _PyThreadStateImpl *tstate = (_PyThreadStateImpl *)PyInterpreterState_ThreadHead(interp);
    struct llist_node *head = &current->asyncio_tasks_head;
    while (tstate) {
        if (tstate != current) {
            llist_concat(head, &tstate->asyncio_tasks_head);
        }
